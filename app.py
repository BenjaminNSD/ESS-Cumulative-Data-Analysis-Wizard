...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            dcc.RadioItems(id='radio', options=[{'label': 'Line', 'value': 'Line'}, {'label': 'Heatmap', 'value': 'Heatmap'}, {'label': 'Bar', 'value': 'Bar'},
                                        {'label': 'Map', 'value': 'Map'}, {'label': 'Sunburst', 'value': 'Sunburst'}], value='Line', labelStyle={'display': 'inline-block'},
                          )],
...
def update_graph(option_slctd, gtype, cntry_opt):
    ...
    if gtype=='Line':
        try:
            fig = px.line(df_sub, x="date", y=option_slctd, color="cntry", hover_name="cntry", line_group="iso3166", line_shape="spline",
                          render_mode="svg", range_x=[2002, 2018]).update_traces(mode='lines+markers')
        except:
            fig = px.line(df_sub, x="date", y=df_sub.index, color="cntry", hover_name="cntry", line_group="iso3166",
                          line_shape="spline", render_mode="svg", range_x=[2002, 2018])
        fig.update_xaxes(tick0=2002, dtick=2, title='')
        try:
            fig.update_layout(
                plot_bgcolor=colors['background'],
        ...
        try:
            ...
            fig = px.bar(df_sub2, x="cntry", y=df_sub2.index, color='cntry', animation_frame='date', hover_name='cntry',
                         orientation='v', title='')
        try:
            fig.update_layout(
                plot_bgcolor=colors['background'],
        ...
        try:
            fig = px.choropleth(df_sub, locations="iso3166", color=option_slctd, hover_name="cntry",
                                color_continuous_scale='reds',
                                scope="europe", projection='eckert4', animation_frame="date")
        except:
            fig = px.choropleth(df_sub, locations="iso3166", color=df_sub.index, hover_name="cntry",
                                color_continuous_scale='reds',
                                scope="europe", projection='eckert4', animation_frame="date")
        try:
            fig.update_layout(
        ...
        try:
            fig = go.Figure(data=go.Heatmap(z=df_sub[option_slctd], x=df_sub.date, y=df_sub.cntry, colorscale='reds', hoverongaps = False))
            fig.update_xaxes(tick0=2002, dtick=2)
            fig.update_yaxes(autorange="reversed", dtick=1, showgrid=False, showline=True)
        except:
            fig = go.Figure(data=go.Heatmap(z=df_sub.index, x=df_sub.date, y=df_sub.cntry, colorscale='reds', hoverongaps = False))
            fig.update_xaxes(tick0=2002, dtick=2)
            fig.update_yaxes(autorange="reversed", dtick=1, showgrid=False, showline=True)
        try:
            fig.update_layout(
